@extends('layouts.admin')

@php
    $title = 'Tambah Menu Paket';
    $pageTitle = 'Tambah Menu Paket';
@endphp

@section('content')
<div class="card shadow-sm border-0 rounded-4">
    <div class="card-body">
        <h2 class="h5 mb-3">Tambah Menu Paket</h2>

        @if ($errors->any())
            <div class="alert alert-danger py-2">
                {{ $errors->first() }}
            </div>
        @endif

        <form action="{{ route('admin.menu.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            
            <input type="hidden" name="is_paket" value="1">
            <!-- Harga beli akan dihitung otomatis di controller -->
            <input type="hidden" name="harga_beli" value="0">

            <div class="mb-3">
                <label class="form-label">Nama Paket</label>
                <input type="text" name="nama" class="form-control" value="{{ old('nama') }}" required>
            </div>

            <div class="mb-3">
                <label class="form-label">Kategori</label>
                <select name="kategori" class="form-select" required>
                    <option value="paket">Paket</option>
                </select>
            </div>

            <div class="row">
                <div class="col-md-6 mb-3">
                    <label class="form-label">Harga Jual Paket</label>
                    <input type="number" name="harga" class="form-control" value="{{ old('harga') }}" required>
                    <div class="form-text text-muted">Harga jual ke pelanggan.</div>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">Diskon (%)</label>
                    <input type="number" name="diskon" class="form-control" min="0" max="100" value="{{ old('diskon', 0) }}">
                </div>
            </div>

            <div class="mb-3">
                <label class="form-label">Stok Paket</label>
                <input type="number" name="stok" class="form-control" value="{{ old('stok', 0) }}" required>
            </div>

            <div class="mb-3">
                <label class="form-label">Deskripsi</label>
                <textarea name="deskripsi" class="form-control" rows="4">{{ old('deskripsi') }}</textarea>
            </div>

            <div class="mb-3">
                <label class="form-label">Gambar Paket</label>
                <input type="file" name="gambar" class="form-control">
            </div>

            <hr class="my-4">

            <h5 class="h6 fw-bold mb-3">Pilih Komponen Penyusun Paket</h5>
            <div class="form-text text-muted mb-3">Centang menu yang termasuk ke dalam paket ini dan atur jumlahnya (Qty). Harga beli paket akan dihitung otomatis dari komponen yang Anda pilih.</div>
            
            <div class="row">
                @foreach($nonPaketMenus as $npm)
                <div class="col-md-6 col-lg-4 mb-3">
                    <div class="border rounded p-2 h-100 d-flex align-items-center gap-2 bg-light">
                        <div class="form-check m-0">
                            <input class="form-check-input check-komponen" type="checkbox" name="komposisi_id_menu[]" value="{{ $npm->id_menu }}" id="menu_{{ $npm->id_menu }}">
                        </div>
                        <div class="flex-grow-1">
                            <label class="form-check-label d-block text-truncate" for="menu_{{ $npm->id_menu }}" style="cursor: pointer;" title="{{ $npm->nama }}">
                                <strong>{{ $npm->nama }}</strong>
                                <br>
                                <small class="text-muted">Rp {{ number_format($npm->harga, 0, ',', '.') }}</small>
                            </label>
                        </div>
                        <div style="width: 70px;">
                            <input type="number" name="komposisi_jumlah[{{ $npm->id_menu }}]" class="form-control form-control-sm qty-input" placeholder="Qty" min="1" value="1" disabled>
                        </div>
                    </div>
                </div>
                @endforeach
            </div>

            <div class="d-flex gap-2 mt-4">
                <button type="submit" class="btn btn-main text-white">Simpan Paket</button>
                <a href="{{ route('admin.menu.index') }}" class="btn btn-outline-secondary rounded-pill">Batal</a>
            </div>
        </form>
    </div>
</div>

<script>
document.addEventListener('DOMContentLoaded', function() {
    const checkboxes = document.querySelectorAll('.check-komponen');
    
    checkboxes.forEach(function(checkbox) {
        checkbox.addEventListener('change', function() {
            // Find the qty input in the same container
            const container = this.closest('.border');
            const qtyInput = container.querySelector('.qty-input');
            
            if (this.checked) {
                qtyInput.disabled = false;
                container.classList.replace('bg-light', 'bg-white');
                container.classList.add('border-primary');
            } else {
                qtyInput.disabled = true;
                container.classList.replace('bg-white', 'bg-light');
                container.classList.remove('border-primary');
            }
        });
    });
});
</script>
@endsection
